...
export const RecipesPage = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await recipesApi.getCollections()
  ...
  return (
    <div className="p-4 space-y-4">
      <h1 className="text-2xl font-bold text-text-primary">Рецепты</h1>
          ...
          <Card key={collection.id} className={clsx("p-4 space-y-3", collection.isInDevelopment && "opacity-90")}>
            ...
            {!collection.isInDevelopment && (
              <div className="flex items-center gap-4 text-sm text-text-secondary">
                <span>🍽️ {collection.recipeCount} рецептов</span>
            ...
            {collection.isInDevelopment ? (
              ...
              <Button fullWidth onClick={() => navigate(`/recipes/${collection.id}`)}>
                Открыть
              </Button>
            ) : (
